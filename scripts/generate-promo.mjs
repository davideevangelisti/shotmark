// Generate Chrome Web Store screenshots (1280x800). Requires `npm run preview`.
...
mkdirSync(resolve(root, "promo"), { recursive: true });
...
await drag("blur", [178, 216], [392, 242]); await page.waitForTimeout(150);
await drag("rect", [170, 154], [392, 188]);
await drag("arrow", [410, 285], [250, 372]);
// enable backdrop through the real UI so the checkbox reflects it
await page.selectOption("#b-bg", "ocean");
await page.locator("#b-enabled").check();
// deselect so no handles show, then fit the whole frame
await page.evaluate(() => {
